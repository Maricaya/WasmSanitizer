...
impl Code{
    pub fn add_fresh_local(&mut self, param_count:usize, ty:ValType) -> Idx<Local>{
...
impl NumericOp {
    pub fn to_type(self) -> FunctionType {
        ...
        match self {
            ...
            I64Extend8S | I64Extend16S | I64Extend32S => FunctionType::new(&[I64], &[I64]), // new instructions
...
impl Module {
    ...
    // Xiao
    pub fn add_imported_function(&mut self, type_: FunctionType, import_module: String, import_name: String, func_name: String) -> Idx<Function> {
        let mut new_func = Function::new_imported(
            type_,
            import_module,
            import_name,
            Vec::new(),
        );
        new_func.name = Some(func_name);
        self.functions.push(new_func);
        (self.functions.len() - 1).into()
    }

    // Xiao
    pub fn add_function_with_name(&mut self, type_: FunctionType, locals: Vec<ValType>, body: Vec<Instr>, name: String) -> Idx<Function> {
        let new_func_idx: Idx<Function> = self.add_function(type_, locals, body);
        if let Some(new_func) = self.functions.last_mut() {
            new_func.name = Some(name);
        }
        (self.functions.len() - 1).into()
    }

    // Xiao
    pub fn remove_function_with_name(&mut self, func_name: String){
        self.functions.retain(|f| !(f.name.as_ref() == Some(&func_name)));
    }
